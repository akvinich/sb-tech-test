package tech.sb.scenario.framework.util;
...
public final class Browser implements WrapsDriver {
    ...
    public List<WebElement> waitForPresenceOfAllElements(By by) {
        return this.getDefaultWait().until(ExpectedConditions.presenceOfAllElementsLocatedBy(by));
    }
    ...
    public int getCountOfElements(By by) {
        return this.waitForPresenceOfAllElements(by).size();
    }
    ...
    public void useActionsMoveToElement(By by) {
        WebElement element = waitForVisibilityOfElement(by);
        new Actions(wrappedWebDriver).moveToElement(element).build().perform();
    }
